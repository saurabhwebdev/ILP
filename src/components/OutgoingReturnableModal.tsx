...
import { CornerRightDown, Plus, Trash2, Calendar } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
interface OutgoingReturnableModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const OutgoingReturnableModal = ({ 
  isOpen, 
  onClose, 
  truck, 
  onProcessingComplete 
}: OutgoingReturnableModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!fromLocation || !toLocation || !invoiceNumber || !gatePassNumber || !takenBy || !authorizeBy || !expectedReturnDate) {
      toast({
        title: "Missing Fields",
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Outgoing Returnable Register completed successfully",
      });
      
      onProcessingComplete();
      onClose();
    } catch (error) {
      console.error("Error processing outgoing returnable register:", error);
      toast({
        title: "Error",
        description: "Failed to process outgoing returnable register",
        variant: "destructive"
      });
    } finally {
      setSubmitting(false);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CornerRightDown className="h-5 w-5 text-green-600" />
            Outgoing Returnable Register
          </DialogTitle>
        </DialogHeader>
        ...
        {/* Outgoing Returnable Form */}
        <div className="border rounded-md p-4 mb-6">
          <h3 className="font-medium text-lg mb-4">Outgoing Returnable Form</h3>
          ...
          {/* Third Row: Expected Return Date, Actual Delivery Date */}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {expectedReturnDate ? format(expectedReturnDate, "PPP") : <span>mm/dd/yyyy</span>}
                  </Button>
                </PopoverTrigger>
            ...
            <div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <Calendar className="mr-2 h-4 w-4" />
                    {actualDeliveryDate ? format(actualDeliveryDate, "PPP") : <span>mm/dd/yyyy</span>}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <CalendarComponent
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Actual Delivery Date</TableHead>
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>{actualDeliveryDate ? format(actualDeliveryDate, "MMM d, yyyy") : 'N/A'}</TableCell>
                        <TableCell>
                          <Button 
                            variant="ghost" 
                            size="icon" 
...
export default OutgoingReturnableModal; 